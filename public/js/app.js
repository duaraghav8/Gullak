...
.factory ('siteUrl', function () {
  return ('http://192.168.0.108:8085');
})
.factory ('api', function () {
  return ({
    getStatus: '/api/status',     //GET
    withdraw: '/api/withdraw',      //GET
    stopService: '/api/stop',       //GET
    startService: '/api/start',    //POST
    getConfig: '/api/start'     //GET
  });
})

.controller('statusCtrl', function($scope, $http, $location, siteUrl, api) {
  $http.get (siteUrl + api.getStatus).then (function (res) { $scope.status = res; }, function (err) {
...
.controller ('createAccountCtrl', function ($scope, $http, siteUrl, api) {
  $http.get (siteUrl + api.getConfig).then (function (res) {
    $scope.recommendedConfig = res.data;
  });
});
//!! IGNORE THE REST !!
